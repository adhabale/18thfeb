...
name;
  ...
  createSanctionGroup = this.formBuilder.group({
    ...
    sanctionList: this.formBuilder.array([])
  });
  ...
  getSanctionActivityType(val){
    this.createSanctionGroup.get('sanctionActivityType').patchValue(val.sanctionActivityType);
    this.createSanctionGroup.get('policyType').patchValue(val.policyType);
    this.createSanctionGroup.get('policyNumber').patchValue(val.policyNumber)
    this.createSanctionGroup.get('claimDate').patchValue(val.claimDate)
    this.createSanctionGroup.get('claimDetails').patchValue(val.claimDetails)
  }

  getSanctionType(val){
    this.createSanctionGroup.get('sanctionFormType').patchValue(val.sanctionFormType);
    this.createSanctionGroup.get('sanctionedCountry').patchValue(val.sanctionedCountry);
    this.createSanctionGroup.get('controlledGoodsInvolvement').patchValue(val.controlledGoodsInvolvement)
    this.createSanctionGroup.get('crimeaExposure').patchValue(val.crimeaExposure)
  }
  ...
  formInitialized(name: string, form: any) {
    ...
    (this.createSanctionGroup.get('sanctionList')as FormArray).push(form);
...
sanctionedCountry: ["Russia"]
 ...
 this.createSanctionGroup.get('sanctionActivityType').setValue(data.sanctionActivityType)
  ...
  onChanges(): void {
this.createSanctionGroup.valueChanges.pipe(debounceTime(500)).subscribe(val => {
